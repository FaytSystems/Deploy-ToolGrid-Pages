...
import {
  ...
  FRONTIER_TOOL_TARGET_COUNT,
  FRONTIER_DOMAINS,
  FRONTIER_OPERATION_FAMILIES,
...
import {
  ...
  NEXT_FRONTIER_TOOL_TARGET_COUNT,
  NEXT_FRONTIER_DOMAINS,
  NEXT_FRONTIER_OPERATION_FAMILIES,
...
import {
  ...
  NEEDED_INHOUSE_TOOL_TARGET_COUNT,
  POTENTIAL_APP_RECORDS,
  POTENTIAL_APP_TARGET_COUNT,
  PRODUCTION_STREAM_ACTIVATION_RECORDS,
  PRODUCTION_STREAM_MADE_RECORDS,
  PRODUCTION_STREAM_TARGET_COUNT,
  SUPPORT_UNLOCKED_APP_TARGET_COUNT,
  buildExpansionToolsSummary,
  buildProductionStreamBatches,
  searchPotentialApps,
  searchProductionStreamApps,
...
import {
  ...
  getSpaceStudioDefaultInputs,
  runSpaceStudioTool,
  verifySpaceStudioSuite
} from "../src/space-studio.js";
import {
  SKELETON_MAX_CELLS,
  SKELETON_TOOL_LABEL,
...
const baseRuntimeForSkeletons = [...tools, ...IN_HOUSE_TOOL_RECORDS, ...MASSIVE_TOOL_FACTORY_RECORDS, ...FUNCTIONAL_TOOL_RECORDS, ...MINI_APP_TOOL_RECORDS, ...DAILY_DEALINGS_PROGRAM_RECORDS, ...FRONTIER_TOOL_RECORDS, ...NEXT_FRONTIER_TOOL_RECORDS, ...NEEDED_INHOUSE_TOOL_RECORDS, ...SPACE_STUDIO_TOOLS, WEBCAM_EXPLORER_TOOL, FACTCHECKER_TOOL, THEME_STUDIO_TOOL, RGB_GENERATOR_TOOL, RGB_MOTION_STUDIO_TOOL, AUDIO_VISUALIZER_STUDIO_TOOL, ...PRICE_TOOLS];
const skeletonTools = buildSkeletonTools(baseRuntimeForSkeletons);
const directWebsiteTools = [...IN_HOUSE_TOOL_RECORDS, ...MASSIVE_TOOL_FACTORY_RECORDS, ...FUNCTIONAL_TOOL_RECORDS, ...MINI_APP_TOOL_RECORDS, ...DAILY_DEALINGS_PROGRAM_RECORDS, ...FRONTIER_TOOL_RECORDS, ...NEXT_FRONTIER_TOOL_RECORDS, ...NEEDED_INHOUSE_TOOL_RECORDS, ...SPACE_STUDIO_TOOLS, WEBCAM_EXPLORER_TOOL, FACTCHECKER_TOOL, THEME_STUDIO_TOOL, RGB_GENERATOR_TOOL, RGB_MOTION_STUDIO_TOOL, AUDIO_VISUALIZER_STUDIO_TOOL, ...PRICE_TOOLS];
const websiteRuntimeTools = [...directWebsiteTools, ...skeletonTools, ...POTENTIAL_APP_RECORDS, ...PRODUCTION_STREAM_ACTIVATION_RECORDS];
const catalogRecordSet = new Set(tools);
const directWebsiteToolsById = new Map(directWebsiteTools.map((tool) => [tool.id, tool]));
const catalogSkeletonReplacementById = new Map(skeletonTools
  .filter((tool) => ids.has(tool.skeletonBaseToolId))
  .map((tool) => [tool.skeletonBaseToolId, tool]));
const websiteToolsById = new Map(websiteRuntimeTools.map((tool) => [tool.id, tool]));
let retiredCatalogCompatibilityAliasCount = 0;
for (const tool of tools) {
  if (websiteToolsById.has(tool.id)) continue;
  const replacement = catalogSkeletonReplacementById.get(tool.id);
  if (replacement) {
    websiteToolsById.set(tool.id, replacement);
    retiredCatalogCompatibilityAliasCount += 1;
  }
}
...
assert.ok(knownEngines.has("inhouse_support_tool"), "needed in-house support tool engine should be available to the runtime");
assert.ok(knownEngines.has("potential_app"), "generated potential app engine should be available to the runtime");
...
assert.equal(DAILY_DEALINGS_VERSION, "2026-06-13-daily-dealings-full-programs-v1");
assert.ok(knownEngines.has("daily_dealings_program"), "daily dealings engine should be available to the runtime");
assert.equal(DAILY_DEALINGS_PROGRAM_RECORDS.length, DAILY_DEALINGS_PROGRAM_TARGET_COUNT, "daily dealings suite should ship every full app/program");
const dailyVerification = verifyDailyDealingsSuite({ knownEngines: engineNames });
assert.equal(dailyVerification.ok, true, `daily dealings verification should pass: ${dailyVerification.errors.slice(0, 8).join("; ")}`);
const dailySummary = buildDailyDealingsSummary();
assert.equal(dailySummary.toolCount, 40, "daily dealings summary should count all programs");
assert.equal(dailySummary.uploadReadyCount, 40, "every daily dealings program should expose upload-ready fields");
assert.ok(searchDailyDealingsPrograms("bill subscription cashflow", { limit: 10 }).some((item) => /Bill|Cashflow/.test(item.tool.name)), "daily search should find bill and cashflow programs");
const dailyTool = DAILY_DEALINGS_PROGRAM_RECORDS.find((tool) => tool.id === "daily-bill-subscription-guardian");
assert.equal(dailyTool.systemLabel, DAILY_DEALINGS_LABEL, "daily dealings tools should carry the daily program label");
assert.match(runDailyDealingsProgram(dailyTool, getDailyDealingsDefaultInputs(dailyTool)), /Money View|Daily Action Board|Uploads attached/, "daily bill program should produce a useful money board");
assert.match(runTool(dailyTool, getDefaultInputs(dailyTool)), /Handoff Pack/, "daily dealings programs should run through the shared engine");
assert.ok(getDailyDealingsToolSchema(dailyTool).some((field) => field.type === "file"), "daily dealings direct schema should expose upload file fields");
assert.ok(getToolSchema(dailyTool).some((field) => field.type === "file"), "daily dealings shared schema should expose upload file fields");
for (const tool of DAILY_DEALINGS_PROGRAM_RECORDS) {
  assert.ok(knownEngines.has(tool.engine), `daily dealings tool ${tool.id} should use a known engine`);
  assert.ok(tool.inHouse, `${tool.id} should be marked as in-house`);
  assert.ok(tool.config?.skeletonPattern?.id, `${tool.id} should expose a skeleton pattern`);
  const output = runTool(tool, getDefaultInputs(tool));
  assert.equal(typeof output, "string", `daily dealings output should be a string for ${tool.id}`);
  assert.ok(output.length > 80, `daily dealings output should not be empty for ${tool.id}`);
}

assert.equal(FRONTIER_TOOL_EXPANSION_VERSION, "2026-06-13-frontier-tool-expansion-v1");
assert.equal(FRONTIER_DOMAINS.length, 400, "frontier tool expansion should add 400 new domains");
assert.equal(FRONTIER_OPERATION_FAMILIES.length, 50, "frontier expansion should add 50 operation families");
assert.equal(FRONTIER_TOOL_RECORDS.length, FRONTIER_TOOL_TARGET_COUNT, "frontier expansion should build every new tool and support tool");
const frontierVerification = verifyFrontierToolSuite({ knownEngines: engineNames });
assert.equal(frontierVerification.ok, true, `frontier verification should pass: ${frontierVerification.errors.slice(0, 8).join("; ")}`);
const frontierSummary = buildFrontierToolSummary();
assert.equal(frontierSummary.toolCount, 20050, "frontier summary should count 20,050 new tools");
assert.ok(searchFrontierTools("wildfire evacuation readiness score", { limit: 12 }).some((item) => /Wildfire Evacuation/.test(item.tool.name)), "frontier search should find new emergency tools");
assert.ok(searchFrontierTools("mobile detailing quote vendor", { limit: 12 }).some((item) => /Mobile Detailing/.test(item.tool.name)), "frontier search should find new niche business tools");
const frontierTool = FRONTIER_TOOL_RECORDS.find((tool) => tool.id === "frontier-wildfire-evacuation-readiness-scorecard");
assert.match(runTool(frontierTool, getDefaultInputs(frontierTool)), /Risk flags|Mitigation plan|Readiness/i, "frontier tools should run through the shared mini-app engine");
assert.ok(getToolSchema(frontierTool).some((field) => field.name === "constraints"), "frontier risk tools should expose relevant schema inputs");

assert.equal(NEXT_FRONTIER_TOOL_EXPANSION_VERSION, "2026-06-13-next-frontier-unrepeated-tools-v1");
assert.equal(NEXT_FRONTIER_DOMAINS.length, 600, "next frontier expansion should add 600 unrepeated domains");
assert.equal(NEXT_FRONTIER_OPERATION_FAMILIES.length, 61, "next frontier expansion should add 61 operation families");
assert.equal(NEXT_FRONTIER_TOOL_RECORDS.length, NEXT_FRONTIER_TOOL_TARGET_COUNT, "next frontier expansion should build every new tool and support tool");
const preNextFrontierTools = [...tools, ...IN_HOUSE_TOOL_RECORDS, ...MASSIVE_TOOL_FACTORY_RECORDS, ...FUNCTIONAL_TOOL_RECORDS, ...MINI_APP_TOOL_RECORDS, ...DAILY_DEALINGS_PROGRAM_RECORDS, ...NEEDED_INHOUSE_TOOL_RECORDS, ...SPACE_STUDIO_TOOLS, ...FRONTIER_TOOL_RECORDS, WEBCAM_EXPLORER_TOOL, FACTCHECKER_TOOL, THEME_STUDIO_TOOL, RGB_GENERATOR_TOOL, RGB_MOTION_STUDIO_TOOL, AUDIO_VISUALIZER_STUDIO_TOOL, ...PRICE_TOOLS, ...POTENTIAL_APP_RECORDS, ...PRODUCTION_STREAM_ACTIVATION_RECORDS];
const nextFrontierVerification = verifyNextFrontierToolSuite({ knownEngines: engineNames, existingTools: preNextFrontierTools });
assert.equal(nextFrontierVerification.ok, true, `next frontier verification should pass: ${nextFrontierVerification.errors.slice(0, 8).join("; ")}`);
assert.equal(findNextFrontierCollisions(preNextFrontierTools).length, 0, "next frontier expansion should not repeat exact ids or names from current tools");
const nextFrontierSummary = buildNextFrontierToolSummary();
assert.equal(nextFrontierSummary.toolCount, 36661, "next frontier summary should count 36,661 unrepeated new tools");
assert.ok(searchNextFrontierTools("museum exhibit provenance evidence", { limit: 12 }).some((item) => /Museum Exhibit Rotation/.test(item.tool.name)), "next frontier search should find cultural archive tools");
assert.ok(searchNextFrontierTools("robot sensor calibration drift", { limit: 12 }).some((item) => /Sensor Calibration Map/.test(item.tool.name)), "next frontier search should find robotics sensor tools");
const nextFrontierTool = NEXT_FRONTIER_TOOL_RECORDS.find((tool) => tool.id === "next-frontier-museum-exhibit-rotation-evidence-quality-grader");
assert.match(runTool(nextFrontierTool, getDefaultInputs(nextFrontierTool)), /Evidence|Quality|Risk flags|Mitigation plan|Next actions/i, "next frontier tools should run through the shared mini-app engine");
assert.ok(getToolSchema(nextFrontierTool).some((field) => field.name === "prompt"), "next frontier tools should expose reusable mini-app schema inputs");

assert.equal(EXPANSION_TOOLS_VERSION, "2026-06-13-needed-tools-and-potential-apps-v1");
assert.equal(NEEDED_INHOUSE_TOOL_RECORDS.length, NEEDED_INHOUSE_TOOL_TARGET_COUNT, "all needed support tools should be made in-house");
assert.equal(POTENTIAL_APP_RECORDS.length, POTENTIAL_APP_TARGET_COUNT, "all generated potential app ideas should become runtime app records");
assert.equal(PRODUCTION_STREAM_ACTIVATION_RECORDS.length, SUPPORT_UNLOCKED_APP_TARGET_COUNT, "every support-unlocked app should get a production build pack");
assert.equal(PRODUCTION_STREAM_MADE_RECORDS.length, PRODUCTION_STREAM_TARGET_COUNT, "production stream should hit the 34k+ made record target");
const expansionVerification = verifyExpansionToolsSuite({ knownEngines: engineNames });
assert.equal(expansionVerification.ok, true, `expansion verification should pass: ${expansionVerification.errors.slice(0, 8).join("; ")}`);
const expansionSummary = buildExpansionToolsSummary();
assert.equal(expansionSummary.totalExpansionRecords, NEEDED_INHOUSE_TOOL_RECORDS.length + POTENTIAL_APP_RECORDS.length + PRODUCTION_STREAM_ACTIVATION_RECORDS.length, "expansion summary should count support tools plus generated apps plus production packs");
assert.equal(expansionSummary.productionStreamMadeCount, PRODUCTION_STREAM_TARGET_COUNT, "expansion summary should count the full production stream");
assert.equal(expansionSummary.productionActivationCount, SUPPORT_UNLOCKED_APP_TARGET_COUNT, "summary should count all support-unlocked production packs");
assert.ok(expansionSummary.byReadiness["ready-with-new-support-tools"] >= 6000, "new support tools should unlock thousands of app records");
const productionBatches = buildProductionStreamBatches();
assert.ok(productionBatches.length >= 130, "production stream should be split into many manageable batches");
assert.equal(productionBatches.reduce((sum, batch) => sum + batch.count, 0), PRODUCTION_STREAM_TARGET_COUNT, "production batches should cover every made record");
assert.ok(searchPotentialApps("retail ecommerce price inventory", { limit: 20 }).some((item) => /Retail|Ecommerce|Price|Inventory/i.test(item.tool.name)), "potential app search should find commerce apps");
assert.ok(searchProductionStreamApps("document intelligence redaction production pack", { limit: 20 }).some((item) => item.tool.productionStreamGenerated), "production stream search should find support-unlocked build packs");
assert.match(runTool(NEEDED_INHOUSE_TOOL_RECORDS.find((tool) => tool.id === "pii-phi-redaction-tool"), getDefaultInputs(NEEDED_INHOUSE_TOOL_RECORDS.find((tool) => tool.id === "pii-phi-redaction-tool"))), /redacted-phone|redacted/i, "PII support tool should redact sensitive fields");
assert.match(runTool(POTENTIAL_APP_RECORDS.find((tool) => tool.id === "potential-retail-and-ecommerce-live-price-intelligence-board"), getDefaultInputs(POTENTIAL_APP_RECORDS.find((tool) => tool.id === "potential-retail-and-ecommerce-live-price-intelligence-board"))), /Live Price Intelligence Board|Current in-house tools wired/, "generated app records should run through shared engine");
assert.ok(getToolSchema(POTENTIAL_APP_RECORDS.find((tool) => tool.config?.appType === "program")).some((field) => field.type === "file"), "generated program records should expose upload file fields");
assert.match(runTool(PRODUCTION_STREAM_ACTIVATION_RECORDS[0], getDefaultInputs(PRODUCTION_STREAM_ACTIVATION_RECORDS[0])), /Production|support|Current in-house tools wired/i, "production build packs should run through the shared engine");
for (const tool of [...NEEDED_INHOUSE_TOOL_RECORDS, ...POTENTIAL_APP_RECORDS.slice(0, 80), ...POTENTIAL_APP_RECORDS.slice(-30), ...PRODUCTION_STREAM_ACTIVATION_RECORDS.slice(0, 40), ...PRODUCTION_STREAM_ACTIVATION_RECORDS.slice(-20)]) {
  assert.ok(knownEngines.has(tool.engine), `expansion tool ${tool.id} should use a known engine`);
  assert.ok((tool.tags || []).length >= 8, `${tool.id} needs dense search tags`);
  const output = runTool(tool, getDefaultInputs(tool));
  assert.equal(typeof output, "string", `expansion output should be a string for ${tool.id}`);
  assert.ok(output.length > 40, `expansion output should not be empty for ${tool.id}`);
}
...
assert.equal(SPACE_STUDIO_VERSION, "2026-06-13-see-in-your-space-studio-v1");
assert.ok(knownEngines.has("space_studio"), "3D Space Studio engine should be available to the runtime");
assert.equal(SPACE_STUDIO_TOOLS.length, 6, "3D Space Studio should ship the first six in-house space programs");
assert.ok(Object.keys(SPACE_OBJECT_PRESETS).length >= 12, "3D Space Studio should include a usable object preset library");
assert.equal(SPACE_STUDIO_MAX_OBJECTS, 64, "3D Space Studio should support a 64-object scene cap");
const spaceVerification = verifySpaceStudioSuite();
assert.equal(spaceVerification.ok, true, `space studio verification should pass: ${spaceVerification.errors.join("; ")}`);
const spaceScene = buildSpaceStudioScene(getSpaceStudioDefaultInputs(SEE_IN_YOUR_SPACE_TOOL), SEE_IN_YOUR_SPACE_TOOL);
assert.ok(spaceScene.objects.length >= 4, "See In Your Space should build a room with scene objects");
assert.ok(spaceScene.uploads.total >= 3, "See In Your Space should preserve upload metadata");
assert.match(runSpaceStudioTool(SEE_IN_YOUR_SPACE_TOOL, getSpaceStudioDefaultInputs(SEE_IN_YOUR_SPACE_TOOL)), /interactive 3D room plan/i, "3D Space Studio should produce a readable plan");
assert.match(runTool(SEE_IN_YOUR_SPACE_TOOL, getDefaultInputs(SEE_IN_YOUR_SPACE_TOOL)), /Scene objects|Uploads attached/, "3D Space Studio should run through the shared tool engine");
assert.ok(getToolSchema(SEE_IN_YOUR_SPACE_TOOL).some((field) => field.type === "file"), "3D Space Studio schema should expose upload file fields");
...
assert.equal(SKELETON_TOOL_VERSION, "2026-06-13-generic-skeleton-tools-v8-daily-dealings-programs");
...
assert.equal(websiteRuntimeTools.filter((tool) => catalogRecordSet.has(tool)).length, 0, "original 2,500 catalog records should be retired from direct website use");
assert.equal(tools.filter((tool) => directWebsiteToolsById.has(tool.id) || catalogSkeletonReplacementById.has(tool.id)).length, tools.length, "every retired catalog tool should resolve to an in-house or skeleton replacement");
assert.equal(catalogSkeletonReplacementById.size, tools.length, "every retired catalog tool should have a generated skeleton replacement");
assert.equal(websiteRuntimeTools.length, 164366, "website runtime should exclude the 2,500 direct catalog records while keeping replacements and app records");
assert.equal(retiredCatalogCompatibilityAliasCount, tools.length - tools.filter((tool) => directWebsiteToolsById.has(tool.id)).length, "retired catalog ids should alias to replacement tools unless an exact in-house replacement already owns the id");
assert.equal(websiteToolsById.get("ad-copy-blank-line-remover").engine, "skeleton_tool", "retired catalog ids should resolve to skeleton replacements for older templates");
...
assert.ok(skeletonReport.patternCount >= 70, "skeleton system should map many generic function patterns including frontier patterns");
