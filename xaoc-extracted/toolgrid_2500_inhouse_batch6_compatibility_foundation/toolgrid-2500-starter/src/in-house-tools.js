...
export const IN_HOUSE_TOOLS_VERSION = "2026-06-13-in-house-tools-compatibility-v6-batch6";
export const IN_HOUSE_TOOL_TARGET_COUNT = 480;
...
const domainTerms = {
  ...
  nursery: ["nursery", "greenhouse", "plants", "propagation", "plant inventory", "watering", "seedlings", "plant care"],
  landscaping: ["landscaping", "lawn care", "route", "crew", "mowing", "mulch", "seasonal cleanup", "yard service"],
  propertymanagement: ["property management", "tenant", "lease", "maintenance request", "inspection", "rent", "unit", "landlord"],
  insurance: ["insurance", "claim", "policy", "coverage", "adjuster", "risk", "deductible", "incident"],
  banking: ["banking", "loan", "payment", "credit", "account", "deposit", "amortization", "financial review"],
  itadmin: ["it admin", "device", "access", "security", "software inventory", "help desk", "asset tag", "password"],
  government: ["government", "permit", "case file", "public record", "department", "compliance", "application", "citizen request"],
  lab: ["lab", "sample", "experiment", "protocol", "specimen", "test result", "research note", "chain of custody"],
  marketplace: ["marketplace", "seller", "listing", "order", "sku", "review", "shipping", "customer question"],
  community: ["community", "member", "forum", "moderation", "event", "announcement", "volunteer", "group"],
  transportation: ["transportation", "logistics", "delivery", "dispatch", "shipment", "fleet", "route", "warehouse"],
  energy: ["energy", "solar", "utility bill", "battery", "panel", "installer", "usage", "savings"],
  hvacplumbing: ["hvac", "plumbing", "service call", "fixture", "equipment", "parts", "maintenance", "aftercare"],
  eventproduction: ["event production", "run of show", "vendor", "guest badge", "sponsor", "av checklist", "venue", "timeline"],
  music: ["music", "audio", "track", "release", "setlist", "royalty", "fan", "credits"],
  esports: ["esports", "gaming", "tournament", "player", "match", "stream", "scrim", "team roster"],
  productivity: ["productivity", "daily plan", "priority", "habit", "goal", "time block", "focus", "task management"],
  homeorg: ["home organization", "declutter", "room inventory", "storage bin", "chore", "household", "supplies", "family"],
  sustainability: ["sustainability", "environment", "waste audit", "carbon", "recycling", "water conservation", "eco", "community"],
  subscription: ["subscription", "membership", "billing", "churn", "renewal", "subscriber", "upgrade", "cancellation"]
};
...
,
  ...
  makeTool({name:"Greenhouse Task Note Cleaner", category:"In-House Nursery & Greenhouse", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean greenhouse task notes before schedules or reports are created", domains:["nursery","operations"], accepts:["greenhouse-notes","task-notes","text"], produces:["clean-greenhouse-notes","document-text","text"]}),
  makeTool({name:"Plant Inventory CSV To JSON", category:"In-House Nursery & Greenhouse", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert plant inventory rows into structured records", domains:["nursery","data","retail"], accepts:["plant-inventory-csv","csv","spreadsheet"], produces:["plant-inventory-json","structured-data","records"]}),
  makeTool({name:"Propagation Batch Label Builder", category:"In-House Nursery & Greenhouse", engine:"markdown_table", purpose:"turn propagation batch records into a simple label table", domains:["nursery","labels"], accepts:["propagation-batch-data","csv","text"], produces:["propagation-label-table","markdown-table","batch-labels"]}),
  makeTool({name:"Watering Schedule Date Tool", category:"In-House Nursery & Greenhouse", engine:"date_add", config:{"unit":"days"}, purpose:"calculate the next watering or greenhouse care date", domains:["nursery","operations"], accepts:["watering-date","schedule-date","text"], produces:["watering-follow-up-date","schedule-date","text"]}),
  makeTool({name:"Plant Care Instruction Checklist", category:"In-House Nursery & Greenhouse", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn plant care notes into a numbered checklist", domains:["nursery","client"], accepts:["plant-care-notes","care-instructions","text"], produces:["plant-care-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Plant SKU Slug Builder", category:"In-House Nursery & Greenhouse", engine:"text_transform", config:{"mode":"slugify"}, purpose:"create a safe SKU or folder slug for a plant listing", domains:["nursery","ecommerce"], accepts:["plant-name","product-title","text"], produces:["plant-sku-slug","project-slug","text"]}),
  makeTool({name:"Greenhouse Yield Average", category:"In-House Nursery & Greenhouse", engine:"simple_formula", config:{"mode":"average"}, purpose:"average greenhouse yield or propagation success numbers", domains:["nursery","data"], accepts:["yield-data","score-data","text"], produces:["greenhouse-yield-average","business-calculation","text"]}),
  makeTool({name:"Nursery Customer Care Reply Formatter", category:"In-House Nursery & Greenhouse", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a plant-care customer reply before sending", domains:["nursery","communication"], accepts:["plant-customer-reply","customer-message","text"], produces:["formatted-plant-care-reply","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Landscape Estimate Scope Cleaner", category:"In-House Landscaping & Lawn Care", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean landscaping estimate notes before pricing or crew planning", domains:["landscaping","client"], accepts:["landscape-scope","client-notes","text"], produces:["clean-landscape-scope","document-text","text"], requiresApproval: true}),
  makeTool({name:"Lawn Route Stop Sorter", category:"In-House Landscaping & Lawn Care", engine:"list_transform", config:{"mode":"sort_az"}, purpose:"sort route stops for lawn care review", domains:["landscaping","operations"], accepts:["route-stops","task-list","text"], produces:["sorted-route-stops","process-outline","text"]}),
  makeTool({name:"Mulch Area Calculator", category:"In-House Landscaping & Lawn Care", engine:"simple_formula", config:{"mode":"area_rectangle"}, purpose:"calculate basic landscape bed square footage", domains:["landscaping","finance"], accepts:["mulch-area-data","area-data","text"], produces:["mulch-area-estimate","business-calculation","text"]}),
  makeTool({name:"Landscape Invoice Total Builder", category:"In-House Landscaping & Lawn Care", engine:"invoice_total", purpose:"calculate landscaping invoice or estimate totals", domains:["landscaping","finance"], accepts:["landscape-invoice-lines","invoice-lines","text"], produces:["landscape-invoice-total","invoice-total","business-calculation","text"], requiresApproval: true}),
  makeTool({name:"Seasonal Cleanup Checklist Builder", category:"In-House Landscaping & Lawn Care", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn seasonal cleanup notes into a crew checklist", domains:["landscaping","operations"], accepts:["cleanup-notes","sop-steps","text"], produces:["cleanup-checklist","process-outline","text"]}),
  makeTool({name:"Crew Safety Message Formatter", category:"In-House Landscaping & Lawn Care", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a crew safety message before sharing", domains:["landscaping","compliance"], accepts:["crew-safety-message","incident-notes","text"], produces:["formatted-crew-safety-message","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Landscape Follow Up Date", category:"In-House Landscaping & Lawn Care", engine:"date_add", config:{"unit":"days"}, purpose:"calculate a landscape service follow-up date", domains:["landscaping","communication"], accepts:["service-date","schedule-date","text"], produces:["landscape-follow-up-date","follow-up-date","text"]}),
  makeTool({name:"Yard Service Table Builder", category:"In-House Landscaping & Lawn Care", engine:"markdown_table", purpose:"turn yard service rows into a readable table", domains:["landscaping","report"], accepts:["yard-service-lines","csv","text"], produces:["yard-service-table","markdown-table","report-section"]}),
  makeTool({name:"Tenant Request Cleaner", category:"In-House Property Management", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean tenant maintenance request notes before routing", domains:["propertymanagement","operations"], accepts:["tenant-request","maintenance-request","text"], produces:["clean-tenant-request","service-ticket","text"], requiresApproval: true}),
  makeTool({name:"Maintenance Request CSV To JSON", category:"In-House Property Management", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert property maintenance requests into structured records", domains:["propertymanagement","data"], accepts:["maintenance-request-csv","csv","spreadsheet"], produces:["maintenance-request-json","structured-data","records"]}),
  makeTool({name:"Unit Inspection Checklist Builder", category:"In-House Property Management", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn unit inspection notes into a checklist", domains:["propertymanagement","compliance"], accepts:["inspection-notes","approval-items","text"], produces:["unit-inspection-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Lease Renewal Date Tool", category:"In-House Property Management", engine:"date_add", config:{"unit":"days"}, purpose:"calculate lease renewal or tenant follow-up dates", domains:["propertymanagement","legal"], accepts:["lease-date","schedule-date","text"], produces:["lease-renewal-date","follow-up-date","text"], requiresApproval: true}),
  makeTool({name:"Rent Ledger Table Builder", category:"In-House Property Management", engine:"markdown_table", purpose:"turn rent ledger rows into a readable table", domains:["propertymanagement","bookkeeping"], accepts:["rent-ledger-lines","csv","text"], produces:["rent-ledger-table","markdown-table","report-section"], requiresApproval: true}),
  makeTool({name:"Property Notice Formatter", category:"In-House Property Management", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a tenant notice or property message before sending", domains:["propertymanagement","communication"], accepts:["property-notice","customer-message","text"], produces:["formatted-property-notice","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Vendor Work Order Numberer", category:"In-House Property Management", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"number vendor work order steps for maintenance", domains:["propertymanagement","homeservices"], accepts:["vendor-work-order","work-order-steps","text"], produces:["numbered-vendor-work-order","process-outline","text"]}),
  makeTool({name:"Property Issue Keyword Finder", category:"In-House Property Management", engine:"keyword_density", purpose:"find repeated maintenance issue terms across property notes", domains:["propertymanagement","research"], accepts:["property-issue-notes","maintenance-request","text"], produces:["property-issue-keywords","tool-search-terms","text"]}),
  makeTool({name:"Claim Note Cleaner", category:"In-House Insurance Admin", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean rough insurance claim notes before review", domains:["insurance","documents"], accepts:["claim-notes","incident-notes","text"], produces:["clean-claim-notes","document-text","text"], requiresApproval: true}),
  makeTool({name:"Claim CSV To JSON", category:"In-House Insurance Admin", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert claim spreadsheet rows into structured records", domains:["insurance","data"], accepts:["claim-csv","csv","spreadsheet"], produces:["claim-json","structured-data","records"]}),
  makeTool({name:"Coverage Checklist Builder", category:"In-House Insurance Admin", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn coverage review notes into a checklist", domains:["insurance","compliance"], accepts:["coverage-notes","approval-items","text"], produces:["coverage-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Deductible Percent Checker", category:"In-House Insurance Admin", engine:"percentage", purpose:"calculate a deductible or coverage percentage check", domains:["insurance","finance"], accepts:["deductible-data","price-data","text"], produces:["deductible-percent","business-calculation","text"], requiresApproval: true}),
  makeTool({name:"Adjuster Follow Up Date", category:"In-House Insurance Admin", engine:"date_add", config:{"unit":"days"}, purpose:"calculate an adjuster or claimant follow-up date", domains:["insurance","operations"], accepts:["claim-date","schedule-date","text"], produces:["adjuster-follow-up-date","follow-up-date","text"], requiresApproval: true}),
  makeTool({name:"Claim Summary Table Builder", category:"In-House Insurance Admin", engine:"markdown_table", purpose:"turn claim rows into a simple claim summary table", domains:["insurance","report"], accepts:["claim-json","csv","text"], produces:["claim-summary-table","markdown-table","report-section"], requiresApproval: true}),
  makeTool({name:"Policyholder Email Formatter", category:"In-House Insurance Admin", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a policyholder email before sending", domains:["insurance","communication"], accepts:["policyholder-email","customer-message","text"], produces:["formatted-policyholder-email","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Risk Term Keyword Finder", category:"In-House Insurance Admin", engine:"keyword_density", purpose:"find repeated risk or incident terms in claim notes", domains:["insurance","research"], accepts:["risk-notes","claim-notes","text"], produces:["risk-keywords","tool-search-terms","text"], requiresApproval: true}),
  makeTool({name:"Loan Intake Note Cleaner", category:"In-House Banking & Loan Desk", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean loan intake notes before review", domains:["banking","documents"], accepts:["loan-intake-notes","client-notes","text"], produces:["clean-loan-intake-notes","document-text","text"], requiresApproval: true}),
  makeTool({name:"Loan Payment Calculator Tool", category:"In-House Banking & Loan Desk", engine:"loan_payment", purpose:"calculate a simple loan payment estimate", domains:["banking","finance"], accepts:["loan-data","price-data","text"], produces:["loan-payment-estimate","business-calculation","text"], requiresApproval: true}),
  makeTool({name:"Account Review CSV To JSON", category:"In-House Banking & Loan Desk", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert account review rows into structured records", domains:["banking","data"], accepts:["account-review-csv","csv","spreadsheet"], produces:["account-review-json","structured-data","records"]}),
  makeTool({name:"Banking Follow Up Date", category:"In-House Banking & Loan Desk", engine:"date_add", config:{"unit":"days"}, purpose:"calculate a customer banking follow-up or review date", domains:["banking","communication"], accepts:["banking-review-date","schedule-date","text"], produces:["banking-follow-up-date","follow-up-date","text"], requiresApproval: true}),
  makeTool({name:"Loan Document Checklist Builder", category:"In-House Banking & Loan Desk", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn loan document requirements into a checklist", domains:["banking","compliance"], accepts:["loan-document-notes","approval-items","text"], produces:["loan-document-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Credit Note Keyword Finder", category:"In-House Banking & Loan Desk", engine:"keyword_density", purpose:"find repeated credit review terms in notes", domains:["banking","research"], accepts:["credit-notes","client-notes","text"], produces:["credit-keywords","tool-search-terms","text"], requiresApproval: true}),
  makeTool({name:"Deposit Summary Table Builder", category:"In-House Banking & Loan Desk", engine:"markdown_table", purpose:"turn deposit or account rows into a simple table", domains:["banking","report"], accepts:["deposit-lines","csv","text"], produces:["deposit-summary-table","markdown-table","report-section"], requiresApproval: true}),
  makeTool({name:"Applicant Message Formatter", category:"In-House Banking & Loan Desk", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a loan applicant message before sending", domains:["banking","communication"], accepts:["applicant-message","customer-message","text"], produces:["formatted-applicant-message","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Device Inventory CSV To JSON", category:"In-House IT Admin", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert device inventory rows into structured IT asset records", domains:["itadmin","data"], accepts:["device-inventory-csv","csv","spreadsheet"], produces:["device-inventory-json","structured-data","records"]}),
  makeTool({name:"Asset Tag Slug Builder", category:"In-House IT Admin", engine:"text_transform", config:{"mode":"slugify"}, purpose:"create a safe device asset tag or folder name", domains:["itadmin","labels"], accepts:["device-name","asset-notes","text"], produces:["asset-tag-slug","project-slug","text"]}),
  makeTool({name:"Access Request Cleaner", category:"In-House IT Admin", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean access request notes before approval", domains:["itadmin","compliance"], accepts:["access-request","approval-items","text"], produces:["clean-access-request","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Help Desk Reply Formatter", category:"In-House IT Admin", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a help desk reply before sending", domains:["itadmin","communication"], accepts:["help-desk-reply","support-reply","text"], produces:["formatted-help-desk-reply","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Software License Checklist", category:"In-House IT Admin", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn software license notes into a review checklist", domains:["itadmin","compliance"], accepts:["software-license-notes","approval-items","text"], produces:["software-license-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Security Incident Keyword Finder", category:"In-House IT Admin", engine:"keyword_density", purpose:"find repeated security incident terms in IT notes", domains:["itadmin","research"], accepts:["security-incident-notes","incident-notes","text"], produces:["security-incident-keywords","tool-search-terms","text"], requiresApproval: true}),
  makeTool({name:"Device Refresh Date Tool", category:"In-House IT Admin", engine:"date_add", config:{"unit":"days"}, purpose:"calculate a device refresh or security review date", domains:["itadmin","operations"], accepts:["device-review-date","schedule-date","text"], produces:["device-refresh-date","schedule-date","text"]}),
  makeTool({name:"IT Asset Table Builder", category:"In-House IT Admin", engine:"markdown_table", purpose:"turn IT asset rows into a readable asset table", domains:["itadmin","report"], accepts:["device-inventory-json","csv","text"], produces:["it-asset-table","markdown-table","report-section"]}),
  makeTool({name:"Permit Application Cleaner", category:"In-House Government Admin", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean permit application notes before review", domains:["government","documents"], accepts:["permit-application","application-notes","text"], produces:["clean-permit-application","document-text","text"], requiresApproval: true}),
  makeTool({name:"Public Record CSV To JSON", category:"In-House Government Admin", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert public record rows into structured records", domains:["government","data"], accepts:["public-record-csv","csv","spreadsheet"], produces:["public-record-json","structured-data","records"]}),
  makeTool({name:"Case File Checklist Builder", category:"In-House Government Admin", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn case file requirements into a numbered checklist", domains:["government","compliance"], accepts:["case-file-notes","approval-items","text"], produces:["case-file-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Citizen Response Formatter", category:"In-House Government Admin", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a citizen response before sending", domains:["government","communication"], accepts:["citizen-response","customer-message","text"], produces:["formatted-citizen-response","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Permit Due Date Calculator", category:"In-House Government Admin", engine:"date_add", config:{"unit":"days"}, purpose:"calculate permit response or review due dates", domains:["government","operations"], accepts:["permit-date","schedule-date","text"], produces:["permit-due-date","schedule-date","text"], requiresApproval: true}),
  makeTool({name:"Department Summary Table Builder", category:"In-House Government Admin", engine:"markdown_table", purpose:"turn department records into a simple summary table", domains:["government","report"], accepts:["department-records","csv","text"], produces:["department-summary-table","markdown-table","report-section"], requiresApproval: true}),
  makeTool({name:"Application Field Inspector", category:"In-House Government Admin", engine:"json_tool", config:{"mode":"keys"}, purpose:"list fields in application records before review", domains:["government","data"], accepts:["application-json","structured-data","records"], produces:["application-field-list","field-list","text"]}),
  makeTool({name:"Public Request Keyword Finder", category:"In-House Government Admin", engine:"keyword_density", purpose:"find repeated themes in public request notes", domains:["government","research"], accepts:["public-request-notes","citizen-response","text"], produces:["public-request-keywords","tool-search-terms","text"], requiresApproval: true}),
  makeTool({name:"Lab Sample CSV To JSON", category:"In-House Academic Research Lab", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert lab sample rows into structured records", domains:["lab","data"], accepts:["lab-sample-csv","csv","spreadsheet"], produces:["lab-sample-json","structured-data","records"]}),
  makeTool({name:"Protocol Step Numberer", category:"In-House Academic Research Lab", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn lab protocol notes into numbered steps", domains:["lab","research"], accepts:["protocol-notes","sop-steps","text"], produces:["numbered-protocol-steps","process-outline","text"], requiresApproval: true}),
  makeTool({name:"Specimen Label Slug Builder", category:"In-House Academic Research Lab", engine:"text_transform", config:{"mode":"slugify"}, purpose:"create a safe specimen label or record slug", domains:["lab","labels"], accepts:["specimen-name","sample-notes","text"], produces:["specimen-label-slug","project-slug","text"]}),
  makeTool({name:"Experiment Notes Cleaner", category:"In-House Academic Research Lab", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean experiment notes before reporting", domains:["lab","documents"], accepts:["experiment-notes","research-notes","text"], produces:["clean-experiment-notes","document-text","text"], requiresApproval: true}),
  makeTool({name:"Sample Average Calculator", category:"In-House Academic Research Lab", engine:"simple_formula", config:{"mode":"average"}, purpose:"average numeric sample observations for a quick lab signal", domains:["lab","data"], accepts:["sample-data","score-data","text"], produces:["sample-average","business-calculation","text"]}),
  makeTool({name:"Chain Of Custody Checklist", category:"In-House Academic Research Lab", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn chain-of-custody notes into a review checklist", domains:["lab","compliance"], accepts:["custody-notes","approval-items","text"], produces:["custody-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Research Result Table Builder", category:"In-House Academic Research Lab", engine:"markdown_table", purpose:"turn test result rows into a readable research table", domains:["lab","report"], accepts:["test-result-lines","csv","text"], produces:["research-result-table","markdown-table","report-section"], requiresApproval: true}),
  makeTool({name:"Lab Review Keyword Finder", category:"In-House Academic Research Lab", engine:"keyword_density", purpose:"find repeated research terms in lab notes", domains:["lab","research"], accepts:["lab-review-notes","research-notes","text"], produces:["lab-review-keywords","tool-search-terms","text"]}),
  makeTool({name:"Marketplace Listing Title Builder", category:"In-House Marketplace Seller", engine:"text_transform", config:{"mode":"titlecase"}, purpose:"clean marketplace product listing titles", domains:["marketplace","ecommerce"], accepts:["marketplace-title","product-title","text"], produces:["clean-marketplace-title","seo-title","text"]}),
  makeTool({name:"Marketplace Order CSV To JSON", category:"In-House Marketplace Seller", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert marketplace order exports into structured records", domains:["marketplace","data"], accepts:["marketplace-order-csv","csv","spreadsheet"], produces:["marketplace-order-json","structured-data","records"]}),
  makeTool({name:"Seller SKU Slug Builder", category:"In-House Marketplace Seller", engine:"text_transform", config:{"mode":"slugify"}, purpose:"create a safe SKU slug for marketplace listings", domains:["marketplace","labels"], accepts:["seller-sku-notes","product-title","text"], produces:["seller-sku-slug","project-slug","text"]}),
  makeTool({name:"Customer Question Reply Formatter", category:"In-House Marketplace Seller", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a marketplace customer question reply before sending", domains:["marketplace","communication"], accepts:["customer-question-reply","customer-message","text"], produces:["formatted-customer-question-reply","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Marketplace Review Keyword Finder", category:"In-House Marketplace Seller", engine:"keyword_density", purpose:"find repeated product review terms for listing improvements", domains:["marketplace","research"], accepts:["marketplace-reviews","customer-feedback","text"], produces:["marketplace-review-keywords","tool-search-terms","text"]}),
  makeTool({name:"Shipping Deadline Date Tool", category:"In-House Marketplace Seller", engine:"date_add", config:{"unit":"days"}, purpose:"calculate shipping or handling deadline dates", domains:["marketplace","operations"], accepts:["order-date","schedule-date","text"], produces:["shipping-deadline-date","schedule-date","text"]}),
  makeTool({name:"Order Summary Table Builder", category:"In-House Marketplace Seller", engine:"markdown_table", purpose:"turn marketplace order rows into a readable order table", domains:["marketplace","report"], accepts:["marketplace-order-json","csv","text"], produces:["marketplace-order-table","markdown-table","report-section"]}),
  makeTool({name:"Listing Price Percent Checker", category:"In-House Marketplace Seller", engine:"percentage", purpose:"calculate a listing discount or fee percentage check", domains:["marketplace","finance"], accepts:["listing-price-data","price-data","text"], produces:["listing-price-percent","business-calculation","text"], requiresApproval: true}),
  makeTool({name:"Community Announcement Formatter", category:"In-House Community Management", engine:"text_transform", config:{"mode":"sentencecase"}, purpose:"clean a group announcement before posting", domains:["community","communication"], accepts:["community-announcement","social-caption","text"], produces:["formatted-community-announcement","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Member CSV To JSON", category:"In-House Community Management", engine:"csv_tool", config:{"mode":"csv_to_json"}, purpose:"convert member list rows into structured records", domains:["community","data"], accepts:["member-csv","csv","spreadsheet"], produces:["member-json","structured-data","records"]}),
  makeTool({name:"Moderation Checklist Builder", category:"In-House Community Management", engine:"list_transform", config:{"mode":"number_lines"}, purpose:"turn moderation notes into a review checklist", domains:["community","compliance"], accepts:["moderation-notes","approval-items","text"], produces:["moderation-checklist","approval-checklist","text"], requiresApproval: true}),
  makeTool({name:"Event RSVP Follow Up Date", category:"In-House Community Management", engine:"date_add", config:{"unit":"days"}, purpose:"calculate an event RSVP or member follow-up date", domains:["community","travel"], accepts:["event-date","schedule-date","text"], produces:["event-rsvp-follow-up-date","follow-up-date","text"]}),
  makeTool({name:"Forum Theme Keyword Finder", category:"In-House Community Management", engine:"keyword_density", purpose:"find repeated discussion themes in community posts", domains:["community","research"], accepts:["forum-posts","member-comments","text"], produces:["forum-theme-keywords","tool-search-terms","text"]}),
  makeTool({name:"Volunteer Role Table Builder", category:"In-House Community Management", engine:"markdown_table", purpose:"turn volunteer role rows into a readable table", domains:["community","nonprofit"], accepts:["volunteer-role-lines","csv","text"], produces:["volunteer-role-table","markdown-table","report-section"]}),
  makeTool({name:"Member Welcome Message Cleaner", category:"In-House Community Management", engine:"text_transform", config:{"mode":"remove_extra_spaces"}, purpose:"clean a member welcome message before sending", domains:["community","communication"], accepts:["member-welcome-message","customer-message","text"], produces:["clean-member-welcome-message","customer-reply","text"], requiresApproval: true}),
  makeTool({name:"Community Rule Slug Builder", category:"In-House Community Management", engine:"text_transform", config:{"mode":"slugify"}, purpose:"create safe slugs for community rules or topic folders", domains:["community","collaboration"], accepts:["community-rule-title","project-title","text"], produces:["community-rule-slug","project-slug","text"]})
,
  makeTool({"name":"Delivery Route CSV To JSON","category":"In-House Transportation & Logistics","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert delivery route rows into structured dispatch records","domains":["transportation","data"],"accepts":["delivery-route-csv","csv","spreadsheet"],"produces":["delivery-route-json","structured-data","records"]}),
  makeTool({"name":"Shipment Label Slug Builder","category":"In-House Transportation & Logistics","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create safe shipment label slugs or folder names","domains":["transportation","labels"],"accepts":["shipment-name","tracking-notes","text"],"produces":["shipment-label-slug","project-slug","text"]}),
  makeTool({"name":"Freight Quote Percent Checker","category":"In-House Transportation & Logistics","engine":"percentage","purpose":"calculate freight margin, surcharge, or quote percentage checks","domains":["transportation","finance"],"accepts":["freight-quote-data","price-data","text"],"produces":["freight-quote-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Dispatch Message Formatter","category":"In-House Transportation & Logistics","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a dispatch or driver message before sending","domains":["transportation","communication"],"accepts":["dispatch-message","customer-message","text"],"produces":["formatted-dispatch-message","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Delivery ETA Date Tool","category":"In-House Transportation & Logistics","engine":"date_add","purpose":"calculate a delivery ETA or dispatch follow-up date","domains":["transportation","operations"],"accepts":["dispatch-date","schedule-date","text"],"produces":["delivery-eta-date","schedule-date","text"]}),
  makeTool({"name":"Load Checklist Builder","category":"In-House Transportation & Logistics","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn load requirements into a numbered checklist","domains":["transportation","operations"],"accepts":["load-notes","approval-items","text"],"produces":["load-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Fleet Maintenance Table Builder","category":"In-House Transportation & Logistics","engine":"markdown_table","purpose":"turn fleet rows into a readable maintenance table","domains":["transportation","report"],"accepts":["fleet-maintenance-lines","csv","text"],"produces":["fleet-maintenance-table","markdown-table","report-section"]}),
  makeTool({"name":"Route Issue Keyword Finder","category":"In-House Transportation & Logistics","engine":"keyword_density","purpose":"find repeated route, delay, or delivery issue terms","domains":["transportation","research"],"accepts":["route-issue-notes","incident-notes","text"],"produces":["route-issue-keywords","tool-search-terms","text"]}),
  makeTool({"name":"Solar Site Notes Cleaner","category":"In-House Solar & Energy","engine":"text_transform","config":{"mode":"remove_extra_spaces"},"purpose":"clean solar site survey notes before planning","domains":["energy","documents"],"accepts":["solar-site-notes","project-notes","text"],"produces":["clean-solar-site-notes","document-text","text"],"requiresApproval":true}),
  makeTool({"name":"Panel Count Quick Calculator","category":"In-House Solar & Energy","engine":"simple_formula","config":{"mode":"average"},"purpose":"calculate a quick panel count or sizing helper from basic numbers","domains":["energy","finance"],"accepts":["solar-sizing-data","score-data","text"],"produces":["panel-count-estimate","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Energy Usage CSV To JSON","category":"In-House Solar & Energy","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert utility usage rows into structured energy records","domains":["energy","data"],"accepts":["energy-usage-csv","csv","spreadsheet"],"produces":["energy-usage-json","structured-data","records"]}),
  makeTool({"name":"Battery Backup Checklist Builder","category":"In-House Solar & Energy","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn battery backup requirements into a checklist","domains":["energy","compliance"],"accepts":["battery-backup-notes","approval-items","text"],"produces":["battery-backup-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Utility Bill Percent Checker","category":"In-House Solar & Energy","engine":"percentage","purpose":"calculate utility bill savings or rate percentage checks","domains":["energy","finance"],"accepts":["utility-bill-data","price-data","text"],"produces":["utility-bill-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Solar Proposal Table Builder","category":"In-House Solar & Energy","engine":"markdown_table","purpose":"turn solar proposal rows into a simple proposal table","domains":["energy","report"],"accepts":["solar-proposal-lines","csv","text"],"produces":["solar-proposal-table","markdown-table","report-section"],"requiresApproval":true}),
  makeTool({"name":"Installer Follow Up Date","category":"In-House Solar & Energy","engine":"date_add","purpose":"calculate installer follow-up or inspection dates","domains":["energy","operations"],"accepts":["installer-date","schedule-date","text"],"produces":["installer-follow-up-date","follow-up-date","text"]}),
  makeTool({"name":"Energy Savings Keyword Finder","category":"In-House Solar & Energy","engine":"keyword_density","purpose":"find repeated savings or energy terms in notes","domains":["energy","research"],"accepts":["energy-savings-notes","research-notes","text"],"produces":["energy-savings-keywords","tool-search-terms","text"]}),
  makeTool({"name":"Service Call Notes Cleaner","category":"In-House HVAC & Plumbing","engine":"text_transform","config":{"mode":"remove_extra_spaces"},"purpose":"clean HVAC or plumbing service call notes","domains":["hvacplumbing","homeservices"],"accepts":["service-call-notes","job-notes","text"],"produces":["clean-service-call-notes","document-text","text"],"requiresApproval":true}),
  makeTool({"name":"Equipment Inventory CSV To JSON","category":"In-House HVAC & Plumbing","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert equipment or fixture inventory rows into structured records","domains":["hvacplumbing","data"],"accepts":["equipment-inventory-csv","csv","spreadsheet"],"produces":["equipment-inventory-json","structured-data","records"]}),
  makeTool({"name":"Service Estimate Percent Checker","category":"In-House HVAC & Plumbing","engine":"percentage","purpose":"calculate labor, parts, or markup percentage checks","domains":["hvacplumbing","finance"],"accepts":["service-estimate-data","price-data","text"],"produces":["service-estimate-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Job Photo Caption Formatter","category":"In-House HVAC & Plumbing","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a job photo caption or note for a service report","domains":["hvacplumbing","media"],"accepts":["job-photo-caption","image-notes","text"],"produces":["formatted-job-photo-caption","asset-caption","text"],"requiresApproval":true}),
  makeTool({"name":"Maintenance Schedule Date Tool","category":"In-House HVAC & Plumbing","engine":"date_add","purpose":"calculate next maintenance or service reminder dates","domains":["hvacplumbing","operations"],"accepts":["maintenance-date","schedule-date","text"],"produces":["maintenance-schedule-date","schedule-date","text"]}),
  makeTool({"name":"Parts List Numberer","category":"In-House HVAC & Plumbing","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn needed parts into a numbered parts list","domains":["hvacplumbing","operations"],"accepts":["parts-list","inventory-notes","text"],"produces":["numbered-parts-list","process-outline","text"]}),
  makeTool({"name":"Service Report Table Builder","category":"In-House HVAC & Plumbing","engine":"markdown_table","purpose":"turn service rows into a readable service report table","domains":["hvacplumbing","report"],"accepts":["service-report-lines","csv","text"],"produces":["service-report-table","markdown-table","report-section"],"requiresApproval":true}),
  makeTool({"name":"Customer Aftercare Message Formatter","category":"In-House HVAC & Plumbing","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a customer aftercare message before sending","domains":["hvacplumbing","communication"],"accepts":["aftercare-message","customer-message","text"],"produces":["formatted-aftercare-message","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Run Of Show Numberer","category":"In-House Event Production","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn event run-of-show notes into numbered steps","domains":["eventproduction","travel"],"accepts":["run-of-show-notes","schedule-notes","text"],"produces":["numbered-run-of-show","process-outline","text"],"requiresApproval":true}),
  makeTool({"name":"Vendor CSV To JSON","category":"In-House Event Production","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert vendor list rows into structured vendor records","domains":["eventproduction","data"],"accepts":["vendor-csv","csv","spreadsheet"],"produces":["vendor-json","structured-data","records"]}),
  makeTool({"name":"Guest Badge Slug Builder","category":"In-House Event Production","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create safe guest badge labels or badge slugs","domains":["eventproduction","labels"],"accepts":["guest-name","badge-notes","text"],"produces":["guest-badge-slug","project-slug","text"]}),
  makeTool({"name":"Timeline Follow Up Date","category":"In-House Event Production","engine":"date_add","purpose":"calculate event timeline follow-up dates","domains":["eventproduction","operations"],"accepts":["event-date","schedule-date","text"],"produces":["timeline-follow-up-date","follow-up-date","text"]}),
  makeTool({"name":"Event Budget Percent Checker","category":"In-House Event Production","engine":"percentage","purpose":"calculate event budget, sponsor, or vendor percentage checks","domains":["eventproduction","finance"],"accepts":["event-budget-data","price-data","text"],"produces":["event-budget-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Sponsor Thank You Formatter","category":"In-House Event Production","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a sponsor thank-you message before sending","domains":["eventproduction","communication"],"accepts":["sponsor-thank-you","customer-message","text"],"produces":["formatted-sponsor-thank-you","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"AV Checklist Builder","category":"In-House Event Production","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn AV and venue needs into a checklist","domains":["eventproduction","operations"],"accepts":["av-notes","approval-items","text"],"produces":["av-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Event Incident Keyword Finder","category":"In-House Event Production","engine":"keyword_density","purpose":"find repeated event incident or venue issue terms","domains":["eventproduction","research"],"accepts":["event-incident-notes","incident-notes","text"],"produces":["event-incident-keywords","tool-search-terms","text"],"requiresApproval":true}),
  makeTool({"name":"Track Metadata Cleaner","category":"In-House Music & Audio","engine":"text_transform","config":{"mode":"remove_extra_spaces"},"purpose":"clean track metadata notes before release","domains":["music","media"],"accepts":["track-metadata","asset-notes","text"],"produces":["clean-track-metadata","document-text","text"]}),
  makeTool({"name":"Setlist Numberer","category":"In-House Music & Audio","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn songs into a numbered setlist","domains":["music","report"],"accepts":["setlist-notes","song-list","text"],"produces":["numbered-setlist","process-outline","text"]}),
  makeTool({"name":"Royalty Split Percent Checker","category":"In-House Music & Audio","engine":"percentage","purpose":"calculate a royalty split or collaborator percentage check","domains":["music","finance"],"accepts":["royalty-split-data","price-data","text"],"produces":["royalty-split-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Release Reminder Date Tool","category":"In-House Music & Audio","engine":"date_add","purpose":"calculate release reminder or promo follow-up dates","domains":["music","marketing"],"accepts":["release-date","schedule-date","text"],"produces":["release-reminder-date","follow-up-date","text"]}),
  makeTool({"name":"Song Notes Keyword Finder","category":"In-House Music & Audio","engine":"keyword_density","purpose":"find repeated lyric, theme, or track-note terms","domains":["music","research"],"accepts":["song-notes","creative-notes","text"],"produces":["song-note-keywords","tool-search-terms","text"]}),
  makeTool({"name":"Release Checklist Builder","category":"In-House Music & Audio","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn release tasks into a checklist","domains":["music","compliance"],"accepts":["release-notes","approval-items","text"],"produces":["release-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Fan Email Formatter","category":"In-House Music & Audio","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a fan email or announcement before sending","domains":["music","communication"],"accepts":["fan-email","customer-message","text"],"produces":["formatted-fan-email","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Track Credits Table Builder","category":"In-House Music & Audio","engine":"markdown_table","purpose":"turn track credit rows into a readable credits table","domains":["music","report"],"accepts":["track-credit-lines","csv","text"],"produces":["track-credits-table","markdown-table","report-section"],"requiresApproval":true}),
  makeTool({"name":"Tournament Checklist Builder","category":"In-House Esports & Gaming","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn tournament setup notes into a checklist","domains":["esports","community"],"accepts":["tournament-notes","approval-items","text"],"produces":["tournament-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Player CSV To JSON","category":"In-House Esports & Gaming","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert player roster rows into structured records","domains":["esports","data"],"accepts":["player-csv","csv","spreadsheet"],"produces":["player-json","structured-data","records"]}),
  makeTool({"name":"Match Recap Formatter","category":"In-House Esports & Gaming","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a match recap before posting","domains":["esports","communication"],"accepts":["match-recap","social-caption","text"],"produces":["formatted-match-recap","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Stream Title Slug Builder","category":"In-House Esports & Gaming","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create a safe stream title slug for folders or links","domains":["esports","creator"],"accepts":["stream-title","project-title","text"],"produces":["stream-title-slug","project-slug","text"]}),
  makeTool({"name":"Prize Pool Percent Checker","category":"In-House Esports & Gaming","engine":"percentage","purpose":"calculate prize split or fee percentage checks","domains":["esports","finance"],"accepts":["prize-pool-data","price-data","text"],"produces":["prize-pool-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Scrim Schedule Date Tool","category":"In-House Esports & Gaming","engine":"date_add","purpose":"calculate scrim, match, or roster deadline dates","domains":["esports","operations"],"accepts":["scrim-date","schedule-date","text"],"produces":["scrim-schedule-date","schedule-date","text"]}),
  makeTool({"name":"Rules Keyword Finder","category":"In-House Esports & Gaming","engine":"keyword_density","purpose":"find repeated rules or dispute terms in gaming notes","domains":["esports","research"],"accepts":["rules-notes","moderation-notes","text"],"produces":["rules-keywords","tool-search-terms","text"],"requiresApproval":true}),
  makeTool({"name":"Team Roster Table Builder","category":"In-House Esports & Gaming","engine":"markdown_table","purpose":"turn roster rows into a readable team table","domains":["esports","report"],"accepts":["team-roster-lines","csv","text"],"produces":["team-roster-table","markdown-table","report-section"]}),
  makeTool({"name":"Daily Brain Dump Cleaner","category":"In-House Personal Productivity","engine":"text_transform","config":{"mode":"remove_extra_spaces"},"purpose":"clean a messy daily brain dump into readable notes","domains":["productivity","documents"],"accepts":["brain-dump","project-notes","text"],"produces":["clean-brain-dump","document-text","text"]}),
  makeTool({"name":"Priority List Numberer","category":"In-House Personal Productivity","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn priorities into a numbered action list","domains":["productivity","intake"],"accepts":["priority-list","task-list","text"],"produces":["numbered-priority-list","waterfall-outline","text"]}),
  makeTool({"name":"Habit Tracker CSV To JSON","category":"In-House Personal Productivity","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert habit tracker rows into structured records","domains":["productivity","data"],"accepts":["habit-tracker-csv","csv","spreadsheet"],"produces":["habit-tracker-json","structured-data","records"]}),
  makeTool({"name":"Goal Deadline Calculator","category":"In-House Personal Productivity","engine":"date_add","purpose":"calculate a goal deadline or review date","domains":["productivity","operations"],"accepts":["goal-start-date","schedule-date","text"],"produces":["goal-deadline-date","schedule-date","text"]}),
  makeTool({"name":"Time Block Table Builder","category":"In-House Personal Productivity","engine":"markdown_table","purpose":"turn time blocks into a readable schedule table","domains":["productivity","report"],"accepts":["time-block-lines","csv","text"],"produces":["time-block-table","markdown-table","report-section"]}),
  makeTool({"name":"Motivation Message Formatter","category":"In-House Personal Productivity","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a personal motivation or reminder message","domains":["productivity","communication"],"accepts":["motivation-message","customer-message","text"],"produces":["formatted-motivation-message","customer-reply","text"]}),
  makeTool({"name":"Task Duplicate Remover","category":"In-House Personal Productivity","engine":"list_transform","config":{"mode":"dedupe"},"purpose":"remove duplicate tasks before the queue is built","domains":["productivity","data"],"accepts":["task-list","waterfall-outline","text"],"produces":["deduped-task-list","waterfall-outline","text"]}),
  makeTool({"name":"Focus Session Slug Builder","category":"In-House Personal Productivity","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create a safe focus-session folder or project slug","domains":["productivity","collaboration"],"accepts":["focus-session-title","project-title","text"],"produces":["focus-session-slug","project-slug","text"]}),
  makeTool({"name":"Room Inventory CSV To JSON","category":"In-House Home Organization","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert room inventory rows into structured household records","domains":["homeorg","data"],"accepts":["room-inventory-csv","csv","spreadsheet"],"produces":["room-inventory-json","structured-data","records"]}),
  makeTool({"name":"Declutter Checklist Builder","category":"In-House Home Organization","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn declutter notes into a numbered checklist","domains":["homeorg","operations"],"accepts":["declutter-notes","task-list","text"],"produces":["declutter-checklist","approval-checklist","text"]}),
  makeTool({"name":"Storage Bin Label Slug Builder","category":"In-House Home Organization","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create safe storage bin labels or folder names","domains":["homeorg","labels"],"accepts":["storage-bin-name","inventory-notes","text"],"produces":["storage-bin-label-slug","project-slug","text"]}),
  makeTool({"name":"Chore Reminder Date Tool","category":"In-House Home Organization","engine":"date_add","purpose":"calculate chore reminder or household maintenance dates","domains":["homeorg","operations"],"accepts":["chore-date","schedule-date","text"],"produces":["chore-reminder-date","schedule-date","text"]}),
  makeTool({"name":"Maintenance Cost Percent Checker","category":"In-House Home Organization","engine":"percentage","purpose":"calculate household maintenance budget percentage checks","domains":["homeorg","finance"],"accepts":["maintenance-cost-data","price-data","text"],"produces":["maintenance-cost-percent","business-calculation","text"]}),
  makeTool({"name":"Home Project Notes Cleaner","category":"In-House Home Organization","engine":"text_transform","config":{"mode":"remove_extra_spaces"},"purpose":"clean home project notes before planning","domains":["homeorg","documents"],"accepts":["home-project-notes","project-notes","text"],"produces":["clean-home-project-notes","document-text","text"]}),
  makeTool({"name":"Family Message Formatter","category":"In-House Home Organization","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a family household message before sending","domains":["homeorg","communication"],"accepts":["family-message","customer-message","text"],"produces":["formatted-family-message","customer-reply","text"]}),
  makeTool({"name":"Household Supply Table Builder","category":"In-House Home Organization","engine":"markdown_table","purpose":"turn household supply rows into a readable table","domains":["homeorg","report"],"accepts":["household-supply-lines","csv","text"],"produces":["household-supply-table","markdown-table","report-section"]}),
  makeTool({"name":"Waste Audit CSV To JSON","category":"In-House Sustainability & Environmental","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert waste audit rows into structured environmental records","domains":["sustainability","data"],"accepts":["waste-audit-csv","csv","spreadsheet"],"produces":["waste-audit-json","structured-data","records"]}),
  makeTool({"name":"Carbon Note Keyword Finder","category":"In-House Sustainability & Environmental","engine":"keyword_density","purpose":"find repeated carbon, waste, or resource terms","domains":["sustainability","research"],"accepts":["carbon-notes","research-notes","text"],"produces":["carbon-note-keywords","tool-search-terms","text"]}),
  makeTool({"name":"Recycling Label Slug Builder","category":"In-House Sustainability & Environmental","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create safe recycling category labels or slugs","domains":["sustainability","labels"],"accepts":["recycling-category","label-notes","text"],"produces":["recycling-label-slug","project-slug","text"]}),
  makeTool({"name":"Sustainability Report Table Builder","category":"In-House Sustainability & Environmental","engine":"markdown_table","purpose":"turn sustainability rows into a readable report table","domains":["sustainability","report"],"accepts":["sustainability-lines","csv","text"],"produces":["sustainability-report-table","markdown-table","report-section"],"requiresApproval":true}),
  makeTool({"name":"Energy Reduction Percent Checker","category":"In-House Sustainability & Environmental","engine":"percentage","purpose":"calculate reduction, diversion, or savings percentages","domains":["sustainability","finance"],"accepts":["energy-reduction-data","price-data","text"],"produces":["energy-reduction-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Conservation Reminder Date Tool","category":"In-House Sustainability & Environmental","engine":"date_add","purpose":"calculate conservation task or inspection dates","domains":["sustainability","operations"],"accepts":["conservation-date","schedule-date","text"],"produces":["conservation-reminder-date","schedule-date","text"]}),
  makeTool({"name":"Eco Action Checklist Builder","category":"In-House Sustainability & Environmental","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn sustainability actions into a checklist","domains":["sustainability","operations"],"accepts":["eco-action-notes","task-list","text"],"produces":["eco-action-checklist","approval-checklist","text"],"requiresApproval":true}),
  makeTool({"name":"Community Eco Email Formatter","category":"In-House Sustainability & Environmental","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean a community eco update before sending","domains":["sustainability","communication"],"accepts":["community-eco-email","customer-message","text"],"produces":["formatted-community-eco-email","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Member Billing CSV To JSON","category":"In-House Subscription & Membership","engine":"csv_tool","config":{"mode":"csv_to_json"},"purpose":"convert member billing rows into structured records","domains":["subscription","data"],"accepts":["member-billing-csv","csv","spreadsheet"],"produces":["member-billing-json","structured-data","records"]}),
  makeTool({"name":"Churn Reason Keyword Finder","category":"In-House Subscription & Membership","engine":"keyword_density","purpose":"find repeated churn or cancellation terms in feedback","domains":["subscription","research"],"accepts":["churn-feedback","customer-feedback","text"],"produces":["churn-reason-keywords","tool-search-terms","text"]}),
  makeTool({"name":"Plan Name Slug Builder","category":"In-House Subscription & Membership","engine":"text_transform","config":{"mode":"slugify"},"purpose":"create a safe subscription plan slug","domains":["subscription","labels"],"accepts":["plan-name","product-title","text"],"produces":["plan-name-slug","project-slug","text"]}),
  makeTool({"name":"Renewal Reminder Date Tool","category":"In-House Subscription & Membership","engine":"date_add","purpose":"calculate renewal reminder or billing follow-up dates","domains":["subscription","communication"],"accepts":["renewal-date","schedule-date","text"],"produces":["renewal-reminder-date","follow-up-date","text"],"requiresApproval":true}),
  makeTool({"name":"Subscription Discount Percent Checker","category":"In-House Subscription & Membership","engine":"percentage","purpose":"calculate subscription discount, churn, or upgrade percentages","domains":["subscription","finance"],"accepts":["subscription-price-data","price-data","text"],"produces":["subscription-discount-percent","business-calculation","text"],"requiresApproval":true}),
  makeTool({"name":"Upgrade Email Formatter","category":"In-House Subscription & Membership","engine":"text_transform","config":{"mode":"sentencecase"},"purpose":"clean an upgrade or renewal email before sending","domains":["subscription","communication"],"accepts":["upgrade-email","customer-message","text"],"produces":["formatted-upgrade-email","customer-reply","text"],"requiresApproval":true}),
  makeTool({"name":"Subscriber Segment Table Builder","category":"In-House Subscription & Membership","engine":"markdown_table","purpose":"turn subscriber segment rows into a readable table","domains":["subscription","report"],"accepts":["subscriber-segment-lines","csv","text"],"produces":["subscriber-segment-table","markdown-table","report-section"],"requiresApproval":true}),
  makeTool({"name":"Cancellation Checklist Builder","category":"In-House Subscription & Membership","engine":"list_transform","config":{"mode":"number_lines"},"purpose":"turn cancellation-save steps into a checklist","domains":["subscription","compliance"],"accepts":["cancellation-notes","approval-items","text"],"produces":["cancellation-checklist","approval-checklist","text"],"requiresApproval":true})
