...
const runtimeTools = Object.freeze([...tools, ...IN_HOUSE_TOOL_RECORDS, WEBCAM_EXPLORER_TOOL, FACTCHECKER_TOOL]);
...
const storageKey = "toolgrid-2500-colab-foundation-v1";
...
function renderResults() {
  ...
  const project = activeProject();
  elements.resultCount.textContent = `${filtered.length.toLocaleString()} matching tools · ${tools.length.toLocaleString()} catalog + ${IN_HOUSE_TOOL_RECORDS.length} in-house + webcam explorer + factchecker · ${CATALOG_VERSION} · ${IN_HOUSE_TOOLS_VERSION}`;
