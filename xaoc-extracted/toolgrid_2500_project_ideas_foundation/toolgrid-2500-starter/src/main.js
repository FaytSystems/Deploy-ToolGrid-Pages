...
function buildProjectTemplateFilters() {
  ...
  for (const [complexity, count] of Object.entries(PROJECT_COMPLEXITY_COUNTS)) {
    ...
    option.textContent = `${complexity.replaceAll("-", " ")} (${count.toLocaleString()})`;
...
function renderProjectTemplates() {
  const templates = searchProjectTemplates(state.templateQuery, {
    limit: maxTemplates,
    complexity: state.templateComplexity
  });
  elements.projectTemplateStats.textContent = `${PROJECT_IDEA_COUNT.toLocaleString()} project ideas · ${PROJECT_TOOL_GROUPS.length} tool groups · ${PROJECT_TEMPLATE_VERSION}`;
  ...
  for (const template of templates) {
    ...
    meta.textContent = `${template.field} · ${template.complexity} · ${template.toolCount} tools · ${template.layout}`;
...
function generateAiProject() {
  ...
  const template = pickProjectTemplateForPrompt(project.prompt);
  applyProjectTemplate(template);
  appendProjectEvent(project, `AI selected the closest match from ${PROJECT_IDEA_COUNT.toLocaleString()} project ideas.`, "success");
}
