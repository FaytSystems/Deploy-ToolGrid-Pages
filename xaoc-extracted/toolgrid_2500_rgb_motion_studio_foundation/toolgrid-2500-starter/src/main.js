...
const baseRuntimeTools = Object.freeze([...tools, ...IN_HOUSE_TOOL_RECORDS, WEBCAM_EXPLORER_TOOL, FACTCHECKER_TOOL, THEME_STUDIO_TOOL, RGB_GENERATOR_TOOL, RGB_MOTION_STUDIO_TOOL]);
...
function renderResults() {
  ...
  const project = activeProject();
  elements.resultCount.textContent = `${filtered.length.toLocaleString()} matching tools · ${tools.length.toLocaleString()} catalog + ${IN_HOUSE_TOOL_RECORDS.length} in-house + webcam explorer + factchecker + theme studio + RGB motion studio + ${SKELETON_TOOL_RECORDS.length.toLocaleString()} skeletons · ${CATALOG_VERSION} · ${IN_HOUSE_TOOLS_VERSION}`;
