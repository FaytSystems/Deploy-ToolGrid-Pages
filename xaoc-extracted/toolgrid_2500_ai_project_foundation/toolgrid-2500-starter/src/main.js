...
const storageKey = "toolgrid-2500-ai-project-foundation-v2";
const projectStorageKey = "toolgrid-2500-project-foundation-v1";
...
function renderAiPlan() {
  ...
  for (const step of plan.steps) {
    ...
    const title = document.createElement("strong");
    title.textContent = `Panel ${step.panelIndex + 1}: ${step.toolName}`;
...
function generateAiProject() {
  ...
  const { count } = parseLayout(state.layout);
  const plan = generateAiProjectPlan({ prompt: project.prompt, tools, maxPanels: Math.min(16, Math.max(count, 4)) });
  project.plan = plan;
  if (!elements.projectInput.value.trim()) project.input = plan.starterPayload || project.prompt;
  else project.input = elements.projectInput.value;
  state.layout = plan.recommendedLayout;
  elements.layout.value = state.layout;
  const planEntries = createPanelEntriesFromPlan(plan, project.id);
  const { count: newCount } = parseLayout(state.layout);
  state.panels = Array.from({ length: newCount }, (_, index) => planEntries[index] || null);
  state.panelFx = {};
  appendProjectEvent(project, `AI generated a ${plan.intentLabel.toLowerCase()} with ${plan.steps.length} compatible tool node${plan.steps.length === 1 ? "" : "s"}.`, "success");
  renderWorkspace();
  renderProjectConsole();
  renderResults();
}
...
async function animateAiProject() {
  ...
  for (const step of plan.steps) {
    ...
    try {
      ...
      const artifact = makeArtifact({ projectId: active.id, panelIndex, tool, inputs, output, runMode: "ai-animated", stepId: step.id });
      appendArtifact(active, artifact);
      payload = artifact.output;
      previousTool = tool;
      entry.aiStepStatus = "done";
      state.panelFx[panelIndex] = "done";
      updatePlanStepStatus(active, step.id, "done", { outputPreview: artifact.output.slice(0, 160) });
    } catch (error) {
      const artifact = makeArtifact({
        projectId: active.id,
        panelIndex,
