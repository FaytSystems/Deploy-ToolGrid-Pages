...
export const PROJECT_FOUNDATION_VERSION = "2026-06-13-ai-project-foundation-v2";
...
const intentProfiles = [
  ...
  },
  {
    ...
    keywords: ["image", "color", "palette", "contrast", "hex", "rgb", "design", "brand", "accessibility", "ui"],
    categories: ["Color", "Converters", "Text"],
    engines: ["color_convert", "contrast_ratio", "text_transform"],
...
export function scoreToolForPrompt(tool, prompt, profile = inferProjectIntent(prompt)) {
...
export function generateAiProjectPlan({ prompt, tools, maxPanels = 8 }) {
  ...
  const scored = tools
    .map((tool) => ({ tool, score: scoreToolForPrompt(tool, cleanPrompt, profile) }))
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || a.tool.name.localeCompare(b.tool.name))
    .map((item) => item.tool);

  const chosen = [];
  const preferredEngines = new Set(profile.engines);
  const directPreferred = scored.filter((tool) => preferredEngines.has(tool.engine));
  const directCategory = scored.filter((tool) => profile.categories.includes(tool.category));
  const fallback = scored;

  for (const bucket of [directPreferred, directCategory, fallback]) {
    for (const tool of bucket) {
      if (chosen.length >= maxPanels) break;
      if (chosen.length === 0 || canConnectTools(chosen[chosen.length - 1], tool) || getToolIO(tool).accepts.includes("text")) {
        chosen.push(tool);
      }
    }
    if (chosen.length >= Math.min(maxPanels, 4)) break;
  }

  const finalTools = uniqueById(chosen).slice(0, Math.max(1, maxPanels));
  const steps = finalTools.map((tool, index) => {
    ...
    return {
      ...
      reason: index === 0
        ? `Starts the ${profile.label.toLowerCase()} from the shared project payload.`
        : connection?.compatible
          ? `Receives ${connection.sourceType} output from ${previous.name}.`
          : "Receives text-compatible project output.",
...
export function runProjectPanelsSimultaneously({ project, panelEntries, toolsById }) {
  ...
  panelEntries.forEach((entry, panelIndex) => {
    ...
    try {
      ...
      artifacts.push(makeArtifact({ projectId: project.id, panelIndex, tool, inputs, output, runMode: "simultaneous", stepId: entry.aiStepId || null }));
    } catch (error) {
...
export function runProjectPanelsAsQueueFrom({ project, panelEntries, toolsById, startPanelIndex = 0, initialPayload = "", runMode = "manual-waterfall" }) {
  ...
  panelEntries.forEach((entry, panelIndex) => {
    ...
    try {
      const inputs = buildInputsFromPayload(tool, payload, payloadType);
      const output = runTool(tool, inputs);
      const artifact = makeArtifact({ projectId: project.id, panelIndex, tool, inputs, output, runMode, stepId: entry.aiStepId || null });
