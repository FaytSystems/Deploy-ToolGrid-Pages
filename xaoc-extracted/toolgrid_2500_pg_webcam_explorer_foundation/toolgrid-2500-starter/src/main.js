...
const runtimeTools = Object.freeze([...tools, ...IN_HOUSE_TOOL_RECORDS, WEBCAM_EXPLORER_TOOL]);
...
function renderResults() {
  ...
  const project = activeProject();
  elements.resultCount.textContent = `${filtered.length.toLocaleString()} matching tools · ${tools.length.toLocaleString()} catalog + ${IN_HOUSE_TOOL_RECORDS.length} in-house + 1 webcam explorer · ${CATALOG_VERSION} · ${IN_HOUSE_TOOLS_VERSION}`;
