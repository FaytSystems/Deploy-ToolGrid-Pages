export const SKELETON_TOOL_VERSION = "2026-06-13-generic-skeleton-tools-v8-daily-dealings-programs";
...
const ENGINE_PATTERNS = Object.freeze({
  ...
  audio_visualizer_studio: { id: "audio-visualizer-compositor", label: "Generic Audio Visualizer Compositor Skeleton", accepts: ["audio", "audio-url", "shape", "lighting", "prompt"], produces: ["visualizer-grid", "loop-plan", "render-plan", "media-preset"], keywords: ["audio", "visualizer", "waveform", "lighting", "shadow", "gif", "loop", "64 variations"] },
  space_studio: { id: "interactive-3d-space", label: "Generic Interactive 3D Space Skeleton", accepts: ["room-photo", "product-image", "model-file", "dimensions", "cad"], produces: ["3d-scene", "placement-plan", "scene-json", "room-layout"], keywords: ["3d", "room", "space", "product placement", "cad", "furniture", "dimensions", "upload"] },
  inhouse_support_tool: { id: "support-capability", label: "Generic In-House Support Capability Skeleton", accepts: ["notes", "rows", "files", "settings"], produces: ["support-output", "workflow-plan", "normalized-data"], keywords: ["support", "parser", "adapter", "workbench", "in-house", "expansion"] },
  daily_dealings_program: { id: "daily-dealings-full-program", label: "Generic Daily Dealings Full Program Skeleton", accepts: ["prompt", "notes", "rows", "files", "dates", "budget"], produces: ["daily-board", "handoff-pack", "program-output"], keywords: ["daily dealings", "full app", "program", "life admin", "handoff", "routine"] },
  mini_app: { id: "functional-mini-app", label: "Generic Functional Mini-App Skeleton", accepts: ["prompt", "notes", "rows", "constraints"], produces: ["mini-app-output", "project-artifact", "workflow-step"], keywords: ["mini app", "daily use", "workflow", "planner", "checklist", "report", "budget", "message"] }
});
...
export function getSkeletonPatternForTool(tool) {
  if (["functional_tool", "mini_app", "daily_dealings_program"].includes(tool?.engine) && tool.config?.skeletonPattern?.id) return tool.config.skeletonPattern;
...
export function buildSkeletonTools(baseTools = []) {
  return baseTools
    .filter((tool) => tool && tool.id && tool.engine !== "skeleton_tool" && tool.toolKind !== "skeleton")
