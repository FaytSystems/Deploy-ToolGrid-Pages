...
export function runFunctionalTool(tool = {}, inputs = {}) {
  ...
  switch (mode) {
    ...
    case "phrase_counter": {
      ...
      const rows = phrases.map((phrase) => [phrase, (text.match(new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length]);
      const top = rows.sort((a, b) => b[1] - a[1])[0] || ["none", 0];
      return ["# Phrase Counter", `Phrases checked: ${phrases.length}`, `Top match: ${top[0]} (${top[1]})`, "", makeMarkdownTable(["Phrase", "Count"], rows)].join("\n");
    }
    case "social_keyword_scanner": {
      ...
      return rows.length
        ? ["# Social Keyword Scanner", `Matches found: ${rows.length}`, "", makeMarkdownTable(["Post", "Keyword", "Excerpt"], rows.slice(0, 64))].join("\n")
        : `# Social Keyword Scanner\nNo keyword matches found.\nKeywords checked: ${keywords.join(", ") || "none"}`;
    }
    case "svg_code_card": {
    ...
    case "label_sheet": {
      ...
      const labels = objects.map((row, index) => [`### Label ${index + 1}`, ...selected.map((field) => `**${field}:** ${row[field] ?? ""}`)].join("\n")).join("\n\n---\n\n");
      return [`# Label Sheet`, `Labels generated: ${objects.length}`, `Fields: ${selected.join(", ") || "all detected fields"}`, "", labels].join("\n");
    }
    case "invoice_builder": {
