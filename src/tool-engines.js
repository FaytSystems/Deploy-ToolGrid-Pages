...
import { FACTCHECKER_MODES, renderFactchecker, runFactcheckerSearch } from "./factchecker-tool.js";
import { renderThemeStudio, renderRgbGenerator, runThemeStudio, runRgbGenerator } from "./theme-system.js";
import { renderRgbMotionStudio, runRgbMotionStudio } from "./rgb-motion-studio.js";
import { renderAudioVisualizerStudio, runAudioVisualizerStudio } from "./audio-visualizer-studio.js";
import { renderPriceComparisonTool, runPriceComparisonSearch, PRICE_COMPARISON_MODES } from "./price-comparison-tool.js";
import { getSpaceStudioDefaultInputs, getSpaceStudioToolSchema, renderSpaceStudio, runSpaceStudioTool } from "./space-studio.js";
import { getSkeletonToolSchema, runSkeletonTool } from "./skeleton-tools.js";
import { getFunctionalDefaultInputs, getFunctionalToolSchema, runFunctionalTool } from "./functional-tools.js";
import { getMiniAppDefaultInputs, getMiniAppToolSchema, runMiniAppTool } from "./mini-apps.js";
import { getDailyDealingsDefaultInputs, getDailyDealingsToolSchema, runDailyDealingsProgram } from "./daily-dealings-programs.js";
import {
  getInHouseSupportDefaultInputs,
  getInHouseSupportToolSchema,
  getPotentialAppDefaultInputs,
  getPotentialAppToolSchema,
  runInHouseSupportTool,
  runPotentialAppTool
...
export function getDefaultInputs(tool) {
  ...
  switch (tool.engine) {
    ...
    case "factchecker":
      return { mode: FACTCHECKER_MODES.QUOTE_VERIFICATION, phrase: "I have a dream", person: "Martin Luther King Jr.", dateFrom: "", dateTo: "", includeSites: "", excludeSites: "", socialTargets: "", count: 16 };
    case "theme_studio":
      return { presetId: "nexus-blueprint", background: "#050A14", box: "#101827", cell: "#13233A", activeCell: "#00E5FF", text: "#E6F7FF", glow: "#7C3CFF", effect: "rotating-outline", speedSeconds: 3.2, brightness: 80, intensity: 75, randomizeEachSignin: false, alternateBetweenLogons: true };
    case "rgb_generator":
      return { effect: "rotating-outline", region: "outer-box", color: "#8A2BFF", speedSeconds: 3.2, brightness: 80, intensity: 75, mode: "continuous" };
    case "audio_visualizer_studio":
      return { audioFileName: "Synthwave_Resonance.mp3", audioUrl: "https://example.com/audio/track.mp3", prompt: "Uplifting synthwave with strong bass, retro arpeggios, and cinematic build.", variationCount: 64, selectedVariation: 27, shapeId: "crystal-nebula", lightingId: "neon-rim", waveSize: 78, width: 64, height: 92, depth: 71, colorIntensity: 85, lightingAngle: 135, shadowStrength: 62, highlightStrength: 88, outlineEffect: "dog-chasing-tail", fillEffect: "plasma-fill", motionSpeed: 1.25, loopLengthSeconds: 15 };
    case "price_comparison":
      return { mode: PRICE_COMPARISON_MODES.EXACT_COMPARE, productName: "wireless earbuds", productUrl: "", productCode: "", brand: "", model: "", minPrice: "", maxPrice: "", color: "", size: "", includeVariations: "false", sources: "", count: 16 };
    case "space_studio":
      return getSpaceStudioDefaultInputs(tool);
    case "functional_tool":
      return getFunctionalDefaultInputs(tool);
    case "mini_app":
      return getMiniAppDefaultInputs(tool);
    case "daily_dealings_program":
      return getDailyDealingsDefaultInputs(tool);
    case "inhouse_support_tool":
      return getInHouseSupportDefaultInputs(tool);
    case "potential_app":
      return getPotentialAppDefaultInputs(tool);
    case "skeleton_tool":
      return { projectGoal: "Apply this reusable skeleton to the active project.", inputPayload: "Project payload", outputShape: "project-output", cells: 8 };
    default:
      return {};
  }
}
  ...
  switch (tool.engine) {
    ...
    case "encoder": {
      ...
      switch (config.mode) {
        ...
        case "base64_decode": {
          try {
            return browserBase64Decode(text);
          } catch {
            return "Input issue: paste a valid Base64 value to decode.";
          }
        }
        ...
        case "url_decode": {
          try {
            ...
            return "Input issue: paste a valid URL-encoded value to decode.";
          }
        }
    ...
    case "json_tool": {
      let value;
      try {
        value = JSON.parse(String(inputs.text || "null"));
      } catch {
        return "Input issue: paste valid JSON, then run this JSON tool again.";
      }
    ...
    case "csv_tool": {
      ...
      if (config.mode === "json_to_csv") {
        ...
        try {
          ...
          return "Input issue: paste valid JSON rows or an object before converting to CSV.";
        }
    ...
    case "factchecker":
      return runFactcheckerSearch(inputs);
    case "theme_studio":
      return runThemeStudio(inputs);
    case "rgb_generator":
      return runRgbGenerator(inputs);
    case "rgb_motion_studio":
      return runRgbMotionStudio(inputs);
    case "audio_visualizer_studio":
      return runAudioVisualizerStudio(inputs);
    case "price_comparison":
      return runPriceComparisonSearch(inputs);
    case "space_studio":
      return runSpaceStudioTool(tool, inputs);
    case "functional_tool":
      return runFunctionalTool(tool, inputs);
    case "mini_app":
      return runMiniAppTool(tool, inputs);
    case "daily_dealings_program":
      return runDailyDealingsProgram(tool, inputs);
    case "inhouse_support_tool":
      return runInHouseSupportTool(tool, inputs);
    case "potential_app":
      return runPotentialAppTool(tool, inputs);
    case "skeleton_tool":
      return runSkeletonTool(tool, inputs);
    case "simple_formula": {
...
export function getToolSchema(tool) {
  const config = tool.config || {};
  if (tool.engine === "skeleton_tool") return getSkeletonToolSchema(tool);
  if (tool.engine === "functional_tool") return getFunctionalToolSchema(tool);
  if (tool.engine === "mini_app") return getMiniAppToolSchema(tool);
  if (tool.engine === "daily_dealings_program") return getDailyDealingsToolSchema(tool);
  if (tool.engine === "inhouse_support_tool") return getInHouseSupportToolSchema(tool);
  if (tool.engine === "potential_app") return getPotentialAppToolSchema(tool);
  if (tool.engine === "space_studio") return getSpaceStudioToolSchema(tool);
  switch (tool.engine) {
    case "text_stats":
    case "text_transform":
    case "list_transform":
    ...
    case "factchecker": return [
      ...
      { name: "phrase", label: "Quoted phrase / keyword", type: "textarea" },
      { name: "person", label: "Person / speaker", type: "text" },
      { name: "dateFrom", label: "From date", type: "date" },
      { name: "dateTo", label: "To date", type: "date" },
      { name: "includeSites", label: "Only these sites/domains", type: "textarea" },
      { name: "excludeSites", label: "Never use these sites/domains", type: "textarea" },
      { name: "socialTargets", label: "Social page URLs / handles", type: "textarea" },
      { name: "count", label: "Cells to prepare", type: "number" }
    ];
    case "theme_studio": return [
      { name: "background", label: "Background color", type: "text" },
      { name: "box", label: "Box color", type: "text" },
      { name: "cell", label: "Cell color", type: "text" },
      { name: "activeCell", label: "Active cell color", type: "text" },
      { name: "glow", label: "Glow color", type: "text" },
      { name: "effect", label: "RGB effect", type: "text" }
    ];
    case "rgb_generator": return [
      { name: "effect", label: "Effect", type: "text" },
      { name: "region", label: "Region", type: "text" },
      { name: "color", label: "Color", type: "text" },
      { name: "speedSeconds", label: "Speed seconds", type: "number" },
      { name: "brightness", label: "Brightness", type: "number" },
      { name: "intensity", label: "Intensity", type: "number" }
    ];
    case "rgb_motion_studio": return [
      { name: "shapeId", label: "Shape id", type: "text" },
      { name: "outlineEffect", label: "Outline effect", type: "text" },
      { name: "fillEffect", label: "Fill effect", type: "text" },
      { name: "outlineColor", label: "Outline color", type: "text" },
      { name: "fillColor", label: "Fill color", type: "text" },
      { name: "layerCount", label: "Effect layers", type: "number" }
    ];
    case "audio_visualizer_studio": return [
      { name: "audioFileName", label: "Audio file name", type: "text" },
      { name: "audioUrl", label: "Audio URL", type: "text" },
      { name: "prompt", label: "Style prompt", type: "textarea" },
      { name: "variationCount", label: "Variations to prepare", type: "number" },
      { name: "shapeId", label: "Shape id", type: "text" },
      { name: "lightingId", label: "Lighting preset", type: "text" },
      { name: "loopLengthSeconds", label: "Loop length seconds", type: "number" }
    ];
    case "price_comparison": return [
      { name: "productName", label: "Product name / description", type: "text" },
      { name: "productUrl", label: "Product URL", type: "text" },
      { name: "productCode", label: "SKU / GTIN / product code", type: "text" },
...
export function renderTool(container, tool, options = {}) {
  ...
  if (tool.engine === "factchecker") {
    renderFactchecker(container, tool, options);
    return;
  }
  if (tool.engine === "theme_studio") {
    renderThemeStudio(container, tool, options);
    return;
  }
  if (tool.engine === "rgb_generator") {
    renderRgbGenerator(container, tool, options);
    return;
  }
  if (tool.engine === "rgb_motion_studio") {
    renderRgbMotionStudio(container, tool, options);
    return;
  }
  if (tool.engine === "audio_visualizer_studio") {
    renderAudioVisualizerStudio(container, tool, options);
    return;
  }
  if (tool.engine === "price_comparison") {
    renderPriceComparisonTool(container, tool, options);
    return;
  }
  if (tool.engine === "space_studio") {
    renderSpaceStudio(container, tool, options);
    return;
  }
  ...
  for (const field of schema) {
    ...
    if (field.type !== "file") input.value = defaults[field.name] ?? "";
  ...
  const collectInputs = () => {
    ...
    for (const field of schema) {
      ...
      if (field.type === "file") {
        ...
        inputs[field.name] = files
          .map((file) => `${file.name} (${Math.round(file.size / 1024)} KB, ${file.type || "unknown"})`)
          .join("\n");
        inputs[`${field.name}Count`] = files.length;
      } else {
        inputs[field.name] = control.value ?? "";
      }
    }
    return inputs;
  };
  const setInputs = (inputs) => {
    for (const [name, value] of Object.entries(inputs || {})) {
      ...
      if (field && field.type !== "file" && "value" in field) field.value = value ?? "";
    }
  };
  const execute = () => {
    try {
